...
#pragma warning restore 0649
        ...
        public class Filter
        {
            ...
            public readonly bool
                allExtensionsHaveSingleSuffix; // 'false' when some extensions have multiple suffixes like ".tar.gz"
        ...
        [SerializeField] private UISkin m_skin;
#if UNITY_EDITOR
...
#pragma warning disable 0649
        [Header("Settings")] [SerializeField] internal int minWidth = 380;
        [SerializeField] internal int minHeight = 300;

        [SerializeField] private float narrowScreenWidth = 380f;

        [SerializeField] private float quickLinksMaxWidthPercentage = 0.4f;

        [SerializeField] private bool sortFilesByName = true;
...
#pragma warning restore 0414
        ...
        [SerializeField] private bool generateQuickLinksForDrives = true;

        [SerializeField] private bool contextMenuShowDeleteButton = true;

        [SerializeField] private bool contextMenuShowRenameButton = true;

        [SerializeField] private bool showResizeCursor = true;
        ...
        [SerializeField] private RectTransform topViewNarrowScreen;

        [SerializeField] private RectTransform middleView;
        private Vector2 middleViewOriginalPosition;
        private Vector2 middleViewOriginalSize;

        [SerializeField] private RectTransform middleViewQuickLinks;
        private Vector2 middleViewQuickLinksOriginalSize;

        [SerializeField] private RectTransform middleViewFiles;

        [SerializeField] private RectTransform middleViewSeparator;
        ...
        [SerializeField] private RectTransform pathInputFieldSlotTop;

        [SerializeField] private RectTransform pathInputFieldSlotBottom;

        [SerializeField] private InputField searchInputField;

        [SerializeField] private RectTransform quickLinksContainer;

        [SerializeField] private ScrollRect quickLinksScrollRect;
        ...
        [SerializeField] private Text filenameInputFieldOverlayText;
        ...
        [SerializeField] private Text filterItemTemplate;
        ...
        [SerializeField] private RectTransform moreOptionsContextMenuPosition;

        [SerializeField] private FileBrowserRenamedItem renameItem;

        [SerializeField] private FileBrowserContextMenu contextMenu;

        [SerializeField] private FileBrowserFileOperationConfirmationPanel fileOperationConfirmationPanel;

        [SerializeField] private FileBrowserAccessRestrictedPanel accessRestrictedPanel;

        [SerializeField] private FileBrowserCursorHandler resizeCursorHandler;
#pragma warning restore 0649
        ...
        private readonly List<string>
            submittedFolderPaths = new List<string>(4); // Used to check if all destination folders have write access

        private readonly List<FileSystemEntry>
            submittedFileEntriesToOverwrite =
                new List<FileSystemEntry>(4); // Existing files selected by the user in save mode
...
#pragma warning restore 0414
        ...
        private bool AllExtensionsHaveSingleSuffix
        {
            get
            {
                return allFiltersHaveSingleSuffix && allExcludedExtensionsHaveSingleSuffix &&
                       m_skin.AllIconExtensionsHaveSingleSuffix;
            }
        }
 ...
 !string.IsNullOrEmpty( parentPath ) && ( FileBrowserHelpers.ShouldUseSAFForPath( parentPath ) || FileBrowserHelpers.DirectoryExists( parentPath ) ); // DirectoryExists: Directory may not be accessible on Android 10+, this function checks that
					}
					else
#endif
                    {
        ...
        private bool
            m_acceptNonExistingFilename =
                false; // Is set to true when showing save dialog for Files or FilesAndFolders, false otherwise
        ...
        private void LateUpdate()
        {
			...
			if( !FileBrowserHelpers.ShouldUseSAF )
#endif
            if (quickLinksInitialized && generateQuickLinksForDrives && m_drivesRefreshInterval >= 0f &&
                Time.realtimeSinceStartup >= drivesNextRefreshTime)
            {
        ...
        {
            get { return validFileEntries.Count; }
        }

        float IListViewAdapter.ItemHeight
        {
            get { return m_skin.FileHeight; }
        }
...
#if !UNITY_EDITOR && UNITY_ANDROID
			string drivesList =
 FileBrowserHelpers.AJC.CallStatic<string>( "GetExternalDrives", FileBrowserHelpers.Context );
        ...
        private void RefreshSkin()
        {
            ...
            for (int i = 0; i < allQuickLinks.Count; i++)
            {
                ...
                allQuickLinks[i].TransformComponent.anchoredPosition = new Vector2(0f,
                    allQuickLinks[i].TransformComponent.GetSiblingIndex() * -m_skin.FileHeight);
            ...
            renameItem.TransformComponent.sizeDelta =
                new Vector2(renameItem.TransformComponent.sizeDelta.x, m_skin.FileHeight);
        ...
        internal void OnContextMenuTriggered(Vector2 pointerPos)
        {
            ...
            Vector2 position;
            RectTransformUtility.ScreenPointToLocalPointInRectangle(rectTransform, pointerPos, canvas.worldCamera,
                out position);
        ...
        public void OnSubmitButtonClicked()
        {
            ...
            if (result == null)
            {
                ...
                else
                {
                    ...
                    for (int startIndex = 0, nextStartIndex = 0;
                         startIndex < filenameInput.Length;
                         startIndex = nextStartIndex)
                    {
                        int filenameLength =
                            ExtractFilenameFromInput(filenameInput, ref startIndex, out nextStartIndex);
                        if (filenameLength == 0)
                            continue;
                        ...
                        if (!VerifyFilename(filename))
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    FileSystemEntry fileEntry = new FileSystemEntry(filename,
                                        FileBrowserHelpers.GetFilename(filename), "", true);
                                ...
                                else if (m_pickerMode != PickMode.Folders && FileBrowserHelpers.FileExists(filename))
                                {
                                    ...
                                    FileSystemEntry fileEntry = new FileSystemEntry(filename, fullPathFilename,
                                        GetExtensionFromFilename(fullPathFilename, AllExtensionsHaveSingleSuffix),
                                        false);
                                    if (FileSystemEntryMatchesFilters(fileEntry, AllExtensionsHaveSingleSuffix))
                                    {
                                        submittedFileEntryPaths.Add(filename);
                            ...
                            if (fileEntryIndex < 0)
                            {
                                if (m_pickerMode != PickMode.Folders)
                                {
                                    ...
                                    if (!m_acceptNonExistingFilename || !isAllFilesFilterActive)
                                    {
                                        ...
                                        {
                                            if (!validFileEntries[i].IsDirectory &&
                                                validFileEntries[i].Name.Length >= filename.Length + 2 &&
                                                validFileEntries[i].Name[filename.Length] == '.')
                                            {
                                                if (validFileEntries[i].Name
                                                    .StartsWith(filename)) // Case-sensitive filename query
                                                {
                                                    fileEntryIndex = i;
                                                    break;
                                                }
                                                else if (textComparer.IsPrefix(validFileEntries[i].Name, filename,
                                                             textCompareOptions)) // Case-insensitive filename query
                                                {
                                    ...
                                    if (m_acceptNonExistingFilename && fileEntryIndex < 0 && !isAllFilesFilterActive)
                                    {
                                        ...
                                        string fileExtension =
                                            GetExtensionFromFilename(filename, AllExtensionsHaveSingleSuffix);
                                        if (string.IsNullOrEmpty(fileExtension) || !filters[filtersDropdown.value]
                                                .MatchesExtension(fileExtension, !AllExtensionsHaveSingleSuffix))
                                        {
                                            filename = Path.ChangeExtension(filename,
                                                filters[filtersDropdown.value].defaultExtension);
            ...
            if (result != null)
            {
                if (m_checkWriteAccessToDestinationDirectory)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(submittedFolderPaths[i]) &&
                            !CheckDirectoryWriteAccess(submittedFolderPaths[i]))
                        {
                ...
                {
                    fileOperationConfirmationPanel.Show(this, submittedFileEntriesToOverwrite,
                        FileBrowserFileOperationConfirmationPanel.OperationType.Overwrite,
...
#endif
                    {
                        multiSelectionPivotFileEntry =
                            Mathf.Clamp(multiSelectionPivotFileEntry, 0, validFileEntries.Count - 1);
        ...
        public void OnItemHeld(FileBrowserItem item)
        {
            ...
            else if (m_allowMultiSelection &&
                     (!item.IsDirectory ||
                      m_pickerMode != PickMode.Files)) // Holding a folder in file selection mode should do nothing
            {
 ...
 FileBrowserHelpers.AJC.CallStatic<string>( "FetchSAFQuickLinks", FileBrowserHelpers.Context );
        ...
        public void RefreshFiles(bool pathChanged)
        {
            ...
            if (pathChanged)
            {
                if (!string.IsNullOrEmpty(m_currentPath))
                    allFileEntries =
                        FileBrowserHelpers.GetEntriesInDirectory(m_currentPath, allExtensionsHaveSingleSuffix);
                else
                    allFileEntries = null;
            }
        ...
        private IEnumerator CreateNewFolderCoroutine()
        {
            ...
            renameItem.Show(string.Empty, m_skin.FileSelectedBackgroundColor, m_skin.FolderIcon, (folderName) =>
            {
                ...
                filesScrollRect.verticalNormalizedPosition = validFileEntries.Count > 1
                    ? (1f - (float)fileEntryIndex / (validFileEntries.Count - 1))
                    : 1f;
            });
        }
        ...
        public void RenameSelectedFile()
        {
            ...
            ((RectTransform)renameItem.transform).anchoredPosition =
                new Vector2(1f, -fileEntryIndex * m_skin.FileHeight);
            renameItem.Show(fileInfo.Name, m_skin.FileSelectedBackgroundColor, GetIconForFileEntry(fileInfo),
                (newName) =>
                {
                    ...
                    if ((fileInfo.IsDirectory && m_pickerMode != PickMode.Files) ||
                        (!fileInfo.IsDirectory && m_pickerMode != PickMode.Folders))
        ...
        public void DeleteSelectedFiles()
        {
            ...
            fileOperationConfirmationPanel.Show(this, validFileEntries, selectedFileEntries,
                FileBrowserFileOperationConfirmationPanel.OperationType.Delete, () =>
                {
                    for (int i = selectedFileEntries.Count - 1; i >= 0; i--)
                    {
        ...
        private bool AddQuickLink(Sprite icon, string name, string path)
        {
            ...
            FileBrowserQuickLink quickLink =
                (FileBrowserQuickLink)Instantiate(quickLinkPrefab, quickLinksContainer, false);
        ...
        internal void OnWindowDimensionsChanged(Vector2 size)
        {
            ...
            float quickLinksWidth = Mathf.Min(middleViewQuickLinksOriginalSize.x,
                windowWidth * quickLinksMaxWidthPercentage);
            ...
            else
            {
                if (pathInputField.transform.parent == pathInputFieldSlotTop)
                {
                    ...
                    middleView.anchoredPosition =
                        middleViewOriginalPosition - new Vector2(0f, topViewAdditionalHeight * 0.5f);
            ...
            for (int i = 0; i < validFileEntries.Count; i++)
            {
                if (validFileEntries[i].Name.Length == filename.Length)
                {
                    if (filename == validFileEntries[i].Name) // Case-sensitive filename query
                        return i;
                    else if (textComparer.Compare(filename, validFileEntries[i].Name, textCompareOptions) ==
                             0) // Case-insensitive filename query
        ...
        private string GetInitialPath(string initialPath)
        {
            if (!string.IsNullOrEmpty(initialPath) && !FileBrowserHelpers.DirectoryExists(initialPath) &&
                FileBrowserHelpers.FileExists(initialPath))
            {
            ...
            if (string.IsNullOrEmpty(initialPath) || !FileBrowserHelpers.DirectoryExists(initialPath))
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(lastBrowsedFolder) &&
                        FileBrowserHelpers.DirectoryExists(lastBrowsedFolder))
        ...
        private bool CheckDirectoryExistsWithTimeout(string path, int timeout = 750)
        {
            ...
            try
            {
                ...
                System.Threading.Tasks.Task task =
                    new System.Threading.Tasks.Task(() => directoryExists = Directory.Exists(path));
                task.Start();
 ...
 new System.Threading.Thread( new System.Threading.ThreadStart( () => directoryExists = Directory.Exists( path ) ) );
				thread.Start();
        ...
        {
            return ShowDialogInternal(onSuccess, onCancel, pickMode, allowMultiSelection, pickMode != PickMode.Folders,
                initialPath, initialFilename, title, saveButtonText);
        }
        ...
        {
            return ShowDialogInternal(onSuccess, onCancel, pickMode, allowMultiSelection, false, initialPath,
                initialFilename, title, loadButtonText);
        }
        ...
        public static IEnumerator WaitForSaveDialog(PickMode pickMode, bool allowMultiSelection = false,
            ...
            string title = "Save", string saveButtonText = "Save")
        {
            if (!ShowSaveDialog(null, null, pickMode, allowMultiSelection, initialPath, initialFilename, title,
                    saveButtonText))
                yield break;
        ...
        //     string title = "Load", string loadButtonText = "Select")
        // {
        //     if (!ShowLoadDialog(null, null, pickMode, allowMultiSelection, initialPath, initialFilename, title,
        //             loadButtonText))
        //         yield break;
        //
        //     while (Instance.gameObject.activeSelf)
        ...
        public static async Task WaitForLoadDialog(PickMode pickMode, bool allowMultiSelection = false,
            string initialPath = null, string initialFilename = null,
            string title = "Load", string loadButtonText = "Select")
        {
            if (ShowLoadDialog(null, null, pickMode, allowMultiSelection, initialPath, initialFilename, title,
